...
This thesis replicates and extends the empirical study **“Informed Enforcement: Lessons from Pollution Monitoring in China”** by Sebastian Axbard and Zichen Deng, published in 2024 in the *American Economic Journal: Applied Economics*. The original paper examines how the introduction of real-time air pollution monitors influenced local enforcement behavior and air quality outcomes in Chinese cities. The study demonstrates that pollution monitoring improves the targeting of inspections, increases enforcement activity, and reduces pollution levels, especially in regions where local officials face strong promotion incentives.
...
- **Interactive Features**  
  ...
  - `data`: Open the data set viewer for exploring tables  
...
- **Info Blocks:**  
  You’ll also find expandable info sections offering explanations on concepts, functions, or data sets relevant to a given task.
...
Quiz: What do you think was a key feature of the 2013 Air Pollution Prevention and Control Action Plan?

(1) Reduction of coal consumption by 50%.
(2) Introduction of real-time air quality monitoring stations.
(3) Implementation of a carbon tax.
(4) Delegation of monitoring to local governments only.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Pollution_Monitoring_China")
```
...
**Pre-Monitoring Phase (Before January 2015)**: Before 2015, there were no air quality monitors in place to systematically transmit pollution data to the central government. In this period, Cities A and B enforced regulations independently, relying solely on locally gathered information. This setup leaves the central government with limited insight into local pollution levels and enforcement activities, potentially creating gaps in oversight and accountability.
...
Quiz: According to the World Health Organization (WHO) data from 2016, what percentage of the world's population lived in areas where air pollution exceeded WHO guidelines?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Air_Pollution_Statistics")
```
...
Quiz: What does the PM2.5 distribution map primarily illustrate?

(1) PM2.5 levels are uniformly distributed across the country.
(2) There is no clear pattern in PM2.5 distribution.
(3) PM2.5 pollution is highest in coastal areas only.
(4) High PM2.5 concentrations are primarily located in the eastern and southern regions.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("PM2.5_Distribution")
```
...
Quiz: What conclusions can be drawn from the distribution of air quality monitoring stations in China?

(1) Monitoring stations are concentrated in regions with higher population density and pollution levels, suggesting targeted efforts in urban and industrial areas.
(2) The distribution of monitoring stations is uniform across the country, ensuring equal coverage in all regions.
(3) Most monitoring stations are located in remote and rural areas to capture background pollution levels.
(4) There is no clear relationship between the placement of monitoring stations and pollution levels.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Monitoring_Stations_China")
```
...
Quiz: What do you think? Was the placement of pollution monitors in China influenced by previous local enforcement activity?

(1) Yes, monitors were placed where previous enforcement was weak.
(2) Yes, monitors were concentrated in high-complaint areas.
(3) No, placement was based on past regulatory patterns.
(4) No, the placement was independent of prior enforcement activity.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("monitor_bias_check")
```
...
In the following exercise, we will take a closer look at the underlying data sets to further examine the recorded air pollution levels and explore their policy implications.


## Exercise 2 -- Data Overview


In this chapter, we provide a detailed overview of the structure and composition of the data used in our analysis. A thorough understanding of the underlying data sets is crucial for interpreting the results and assessing the credibility of our empirical approach.
...
Next, we focus on the primary data set used throughout the analysis, referred to as *firm_enf.dta*. This data set links firm-level enforcement records with spatial, temporal, and environmental variables. Since related data sets at the city and monitor levels are largely derived from the same structure and share key variables, understanding *firm_enf.dta* provides a strong foundation for interpreting the empirical results and conducting subsequent analyses.
...
The data set used in this analysis comprises several key sources, categorized into proprietary and public data. The proprietary data is available only in anonymized versions, with all identifying information removed. Each source contributes unique insights, enabling a comprehensive evaluation of air pollution trends and enforcement activities.

The Proprietary Data consists of:

**Enforcement Records:**
The enforcement data, obtained from the *Institute of Public & Environmental Affairs* (IPE), monitors regulatory compliance in China and contains anonymized information on actions taken against firms violating air quality regulations, such as fines, warnings, production suspensions, and equipment upgrades. As documented in Axbard and Deng (2024), the data set includes **55,184** enforcement records collected between 2010 and 2017 across **177** prefecture-level cities, providing a detailed basis for classifying violation types (air, water, waste, procedural) and assessing the intensity of regulatory actions.


**Firm Characteristics:**
In addition to enforcement data, the data set includes firm-level information from the *Annual Survey of Industrial Firms* (ASIF), conducted by the *National Bureau of Statistics*. It contains detailed data on firms’ economic performance, including revenue, production, employment, energy use, and environmental spending. This allows us to examine how firm characteristics relate to regulatory compliance and how enforcement impacts firm outcomes. As with the enforcement data, all identifiers have been removed to ensure confidentiality.
...
The data set includes daily air pollution measurements obtained from China’s *Ministry of Environmental Protection* (MEP). This data provides detailed records of PM2.5 concentrations across a nationwide network of monitoring stations.
...
**Weather Data:**
Weather conditions significantly influence air pollution levels. To control for these effects, our data set includes meteorological variables from the *China Meteorological Administration*, such as temperature, humidity, wind, and precipitation. While we do not directly analyze weather patterns in our work, incorporating this data allows us to account for natural variation in air quality and better isolate the effects of regulatory interventions.


**Aerosol Optical Depth (AOD) Data:**
In addition to ground-based pollution measurements, the data set includes satellite-derived pollution indicators from *NASA*, specifically Aerosol Optical Depth (AOD) data. AOD measures the density of particulate matter in the atmosphere by detecting the scattering and absorption of sunlight by airborne particles.
...
In the next step, we will introduce one specific data set in more detail to lay the groundwork for our analysis.



## Exercise 2.2 -- Introduction to the Data Set

In this section, we introduce the core data set used in our analysis, providing an overview of its structure, key variables, and sources. A solid understanding of the data’s composition is essential for accurately interpreting the results and evaluating the effects of air pollution monitoring and enforcement in China.

We begin our analysis by focusing on the data set *firm_enf.dta*, as it also serves as the starting point for the original authors. This data set forms a central foundation of the study and provides a representative overview of the overall data structure. Many of the other data sets used throughout the analysis are closely linked to or derived from *firm_enf.dta*, and they share a similar structure. In particular, they contain the same key variables that are crucial for the broader empirical investigation.
...
The data set is provided in *.dta* format and the accompanying code is written for *Stata version 17*. Since base *R* does not natively support *.dta files*, we use the `read_dta()` function from the `haven` package to import the data. A brief explanation of how this function works is provided in the info box below for those unfamiliar with it.
...
**Remark:** The file *firm_enf.dta* turned out to be quite large and caused persistent issues during the loading process, particularly when used within a Shiny application. In multiple test runs, the app experienced long loading times, frequent disconnections, and occasional freezing. These issues are likely due to the way Shiny handles large files in memory.
For this reason, we will no longer work with the original `.dta` file in the following sections. Instead, we use a pre-processed and memory-efficient `.rds` version (*firm_enf_clean.rds*), which contains the same content but loads significantly faster and more reliably.
The information about the `.dta` file format and the `haven` package has intentionally been kept in the problem set, as it still provides helpful context for learners who may work with Stata data in other applications or environments.

**Note (added afterwards):** Despite these adjustments, stability issues may still occur when using the Shiny version on shinyapps.io, especially due to automatic reconnection attempts triggered by file size. If that happens, just skip this and *Chapter 3*, as all exercises there rely on the *firm_enf data set*.

To avoid repeated delays and improve performance, I have converted all original data sets from the paper into the *.rds* format. As a format native to *R*, *.rds* allows for significantly faster and more efficient processing of large data sets, resulting in a smoother workflow throughout the analysis.
...
Now, let us load the data set in its *.rds* format, which offers faster loading and better performance in *R*. We use the `read_rds()` function from *R's* `readr` package.

**Task:** First, load the `readr` package. Then, use `read_rds()` to read the file *firm_enf_clean.rds* and save it in a variable called `dat`. 

```{r "7_5"}
#Load the readr package and use read_rds() to load "firm_enf_clean.rds" into the variable dat
```

**Note:** The *firm_enf* data set, originally nearly 2 GB as a *DTA* file, was reduced to under 30 MB by converting it to *RDS* and removing variables irrelevant to our analysis. This streamlined version improves performance and ensures cleaner outputs.

With the data set successfully loaded into `dat`, we can now take a closer look at its structure and the parameters it contains.

There are multiple ways to gain an overview of a data set. One common approach is using the `head()` function in *R*, which displays the first six rows of `dat`. Alternatively, we can examine a random subset of six rows using the `sample_n(data, rows)` function. These methods help us familiarize ourselves with the variables and better understand how the data set is organized.

**Task:** Apply the `head()` function to `dat` and set the number of rows (`n`) to 3, in order to display only the first three observations of the data set.

```{r "7_6"}
#use head() to show the first three rows of dat
```
These are the first three rows of our data set, highlighting some of the key variables it contains. Each row represents a firm-level observation for a specific year and quarter. The data set links firms to the nearest air quality monitor, combining information on their location, industry, revenues, and employment with regulatory enforcement actions and pollution exposure. This structure allows us to analyze how proximity to monitoring stations affects local environmental enforcement.
...
The data set also includes key environmental variables that influence pollution levels and dispersion. The `tem_mean` column records the average temperature, which affects air pollution dynamics, such as smog formation and pollutant accumulation.
...
So, let us see what happens when we filter the data set for the year 2015 to examine differences in the `post1` column. This will allow us to analyze variations in enforcement and pollution levels before and after the introduction of monitoring.
...
After loading the `dplyr` package, we now want to filter the data set for the year 2015 and display the first three rows.

**Task:** Fill in the blanks to filter the data set for the year 2015 and display the first three rows again.

```{r "7_11"}
# Filter the data set to include only observations from the year 2015 
...
Quiz: What does the `post1` column show after filtering the data set for the year 2015?

(1) The post1 values are still a mix of 0 and 1, reflecting both pre- and post-monitoring data.
(2) All post1 values are now 1, meaning all observations fall within the post-monitoring period.
(3) The number of post1 = 0 values has increased, indicating more pre-monitoring observations.
(4) The post1 variable is missing from the filtered data set, suggesting a processing error.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Observing_Post1_Changes")
```
...
In this sub-exercise, we took a closer look at the first data set, which serves as a foundational building block for the analyses that follow. By exploring its structure and identifying key variables related to pollution enforcement and monitoring, we established the basis for evaluating policy impacts in the upcoming chapters.
...
```
In the previous chapter, we explored the structure of the *firm_enf_clean* data set. Now, we want to analyze how the placement of pollution monitors relates to enforcement actions by examining whether proximity to a monitor influences the likelihood of regulatory enforcement.
...
To begin, we will filter the dat aset to include only firms located within 50 km of a monitor and those that started operations in 2010 or earlier.

**Task:** Replace the ___ placeholders with the correct parameter to filter the data set and save it to `dat_filtered`. 
...
Use the `mutate()` function from the `dplyr` package to create the new column. This function allows you to modify or add variables to a data set efficiently.
...
Quiz: What does the enforcement gradient graph primarily show about the relationship between distance to monitors and enforcement likelihood?

(1) Enforcement was consistently high regardless of distance or time period.
(2) Distance to monitors had no effect on enforcement likelihood.
(3) Enforcement increased significantly near monitors after their introduction (2015–2017).
(4) The introduction of monitors reduced enforcement activities overall.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Enforcement_Gradient")
```
...
Quiz: What does the graph reveal about enforcement trends?

(1) Higher enforcement far from monitors.
(2) No change after 2015.
(3) Random enforcement patterns.
(4) Similar trends before 2015, sharper increase near monitors after 2015.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Enforcement_Trends")
```
...
Quiz: Which firms are defined as the treatment group in the difference-in-differences (DiD) framework used in this study? This was briefly discussed in the previous chapter.

(1) Firms located within 10 km of a monitor
(2) Firms located beyond 10 km of a monitor
(3) Firms in the same industry as those near a monitor
(4) Firms with higher-than-average pollution levels

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("treatment_control_group")
```
...
Quiz: What does a higher likelihood of enforcement actions for firms near pollution monitors indicate about the treatment effect?

(1) The treatment effect is negative.
(2) There is no treatment effect.
(3) The treatment effect is positive.
(4) The treatment effect is spurious.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("pollution_monitoring_treatment_effect")
```
...
In this section, we will manually apply the difference-in-differences (DiD) formula to our data set. The goal is to compute the average enforcement probability for both the treatment and control groups, before and after the introduction of pollution monitoring. This results in four mean values, which will then be used to calculate the treatment effect.

To keep things clear, we will break the process into two steps. First, we calculate the group averages as defined by the DiD setup. Then, we use these averages to compute the final DiD estimator, which captures the effect of pollution monitoring on enforcement actions.

Before we proceed, we’ll load the data set. From this point on, we’ll be working with a pre-filtered version that I’ve already cleaned and transformed based on the adjustments introduced in the previous chapter. Variables such as the minimum distance to the nearest monitor, the monitor start year, and other key filters have already been applied. This allows us to skip repetitive preprocessing steps and focus directly on the analysis.
...
Before moving on to the final step of computing the DiD estimator, let us quickly check the extracted values to ensure they are correctly assigned and consistent with our expectations.
...
Quiz: Based on the extracted mean enforcement probabilities, what can we conclude about the enforcement trend?

(1) The control group experienced a stronger increase in enforcement probability compared to the treatment group.
(2) The treatment group experienced a stronger increase in enforcement probability compared to the control group.
(3) Both groups saw the same increase in enforcement probability.
(4) Enforcement probability remained unchanged over time.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("did_enforcement_increase")
```
...
Quiz: The manually computed difference-in-differences (DiD) estimator is **0.00248**. How should we interpret this result?

(1) After the introduction of pollution monitoring, the average enforcement probability for firms within 10 km of a monitor increased by 0.00248 more compared to firms further away.
(2) The overall enforcement probability increased by 0.00248 for all firms after monitoring was introduced.
(3) After the introduction of pollution monitoring, the average enforcement probability for both treated and control firms increased by 0.00248 more compared to a world without monitoring.
(4) The overall enforcement probability decreased by 0.00248 for all firms after monitoring was introduced.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("did_interpretation")
```
...
Let us now reload the data set and begin with the regression analysis.

**Task:** Read the file *filtered_firm_enf.rds* with the help of `readRDS()` and save it in `dat`.
...
Quiz: What does the coefficient $β₃$ in the difference-in-differences (DiD) regression represent?

(1) The overall difference in enforcement probability before and after monitoring was introduced.
(2) The baseline enforcement probability for firms farther than 10 km from a monitor.
(3) The general trend of enforcement actions over time, regardless of monitoring.
(4) The causal effect of pollution monitoring on the probability of enforcement actions for firms within 10 km of a monitor.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("did_regression")
```
...
Quiz: Which variables are the independent ones in our difference-in-differences (DiD) regression?
...
Quiz: Does the estimated coefficient of the interaction term match the manually computed difference-in-differences (DiD) estimator?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("did_value_match")
```

The interaction term’s coefficient **(0.00248)**, which captures the difference-in-differences (DiD) effect, matches the result from the manual calculation. It is statistically significant at the 1% level (p < 0.01), indicating a meaningful impact of pollution monitoring on enforcement actions.
...
Quiz: What does the intercept **(0.00207)** represent in the regression?

(1) The baseline enforcement probability for firms farther than 10 km from a monitor before the monitoring program started.
(2) The overall probability of enforcement actions for all firms.
(3) The probability of enforcement actions after the monitoring program started.
(4) The increase in enforcement probability for firms within 10 km of a monitor.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("did_intercept")
```
...
To connect the theoretical foundation of standard errors with empirical practice, we now turn to a hands-on analysis using the *filtered_firm_enf.rds* data set again. In the following tasks, we revisit the regression model from *Exercise 3.2* and examine whether the key assumptions behind standard errors, such as homoskedasticity and independence, hold in this context.


**Task:** Press `check` to read the file *filtered_firm_enf.rds* with the help of `readRDS()` and save it in `dat`.
...
Quiz: Why does this plot support the use of cluster-robust standard errors?

(1) Because the average residuals are always zero.
(2) Because clustering ensures better model fit.
(3) Because residual variation appears unequal across cities, violating homoskedasticity.
(4) Because the regression coefficients change when clustering is applied.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Heteroskedasticity.boxplot2")
```
...
Quiz: What is the main difference between the regression output of `lm()` and `feols()` when clustering on `city_id`?

(1) feols() can display different types of R² values depending on model specification, while lm() only reports a single R².
(2) feols() reports cluster-robust standard errors that account for within-city correlations, whereas lm() uses conventional standard errors.
(3) feols() does not support clustering of standard errors.
(4) feols() automatically reduces coefficient magnitudes when clustering is applied.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("clustered_se_feols")
```
...
Quiz: What is the main effect of clustering standard errors at the city level?

(1) It changes the estimated coefficients.
(2) It reduces the number of observations.
(3) It increases the standard errors, making inference more robust.
(4) It improves the model fit by increasing R².

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Impact of Clustering on Standard Errors")
```
...
Quiz: What does a common trend between treatment and control groups before 2015 indicate?

(1) That the difference-in-differences (DiD) method is likely to provide causally valid estimates.
(2) That the policy had an impact before 2015.
(3) That pollution levels in treatment and control cities are always identical.
(4) That both groups experienced a sudden drop in pollution levels after 2015.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("parallel_trends_assumption")
```
...
Let us now check this assumption by visualizing the pre-policy enforcement trends for both groups.

I have already completed the data preparation, so we can now directly load the processed data set. As in the previous section, we constructed an interaction term (`min_dist_10_post1`) to capture the treatment effect. This variable combines the treatment group indicator (`min_dist_10`) with the post-treatment period (`post1`), allowing us to estimate the differential effect after the intervention.

To visualize trends in enforcement, we calculated the average enforcement probability (`any_air`) for each year and treatment group. This was done by grouping the data by `year` and `min_dist_10`, computing the mean of the outcome variable while excluding missing values. The resulting summary data set is now ready for visualization and further analysis.

**Task:** Press `check` to load the data set *dat.mean.rds* using `readRDS()` and assign it to the variable named `dat.mean`. 
...
Quiz: Before the policy intervention in 2015, what does the parallel trends assumption imply about the enforcement probability trends for the treatment and control groups?

(1) The treatment group had a significantly higher enforcement probability than the control group before 2015.
(2) The control group had a significantly higher enforcement probability than the treatment group before 2015.
(3) Both groups followed a similar enforcement probability trend before 2015.
(4) The enforcement probability fluctuated randomly between the two groups before 2015.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Parallel Trends and Policy Impact")
```
...
Quiz: What do you think is the primary purpose of fixed effects in econometric models?

(1) To eliminate bias caused by unobserved, time-invariant factors.
(2) To estimate the effects of time-invariant characteristics.
(3) To reduce the correlation between independent variables.
(4) To make the model more complex without any real benefit.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("fixed_effects_quiz")
```
...
Quiz: The estimated coefficient $β_1$ represents the effect of a firm being within 10 km of a pollution monitor on enforcement probability. How should this be interpreted?

(1) The enforcement probability increases over time for all firms, regardless of distance to a monitor.
(2) Firms within 10 km of a monitor experience a higher enforcement probability compared to those farther away, holding fixed effects constant.
(3) β1 captures the overall enforcement probability level in the post-monitoring period.
(4) β1 reflects time-invariant firm characteristics that influence enforcement probability.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("interpretation_beta1_advanced")
```
...
**Task:** Load the data set *filtered_firm_enf.rds* and store it as `dat` for use in the upcoming regression analysis.
...
Next, we will again transform the data set by creating a new variable, `min_dist_10_post1`, which will be the product of `min_dist_10` and `post1`. We will also generate a grouping variable, `ind_time`, based on the combination of `industry`, `prov_id`, and `time`, allowing us to uniquely identify each group.



**Task:** Fill in the blanks to correctly group the data set. Complete the `group_by()` function with the appropriate variables before creating `ind_time`. The correct grouping variables are `industry`, `prov_id`, and `time`.
...
**Task:** Press `check` to display a random sample of rows from the data set.
...
Quiz: By what percentage does the presence of a monitor increase the probability of an air-pollution-related enforcement action?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("monitor_enforcement_increase")
```
...
Quiz: What do the results for the `Warning` enforcement action indicate?

(1) Proximity to a monitor significantly increases the likelihood of receiving a warning.
(2) Warnings are the most common enforcement action influenced by monitors.
(3) Warnings are completely eliminated for firms near monitors.
(4) Proximity to a monitor has no significant effect on the likelihood of receiving a warning.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Understanding_Warning_Results")
```
...
Quiz: Based on this Table, what determines the minimum number of monitors assigned to a city?

(1) The pollution levels in the city determine the number of monitors assigned.
(2) The economic activity and industrial output of a city are the primary factors for monitor assignment.
(3) Monitors are assigned randomly across cities.
(4) The population and the size of the built-up area of a city determine the minimum number of monitors assigned.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Monitor Assignment Criteria")
```
...
**Task:** Press `check` to load the data sets *city_info.rds* and *share.rds* into the variables `city_info` and `share_data`, and display the first three rows of each data set using `head()`.
...
Quiz: What do the vertical lines in the plot represent?

(1) The confidence intervals around the estimated percentage of high-pollution activity.
(2) The number of monitors in each city.
(3) The range of pollution levels in different areas.
(4) The distribution of monitors across different locations.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Understanding_Confidence_Intervals")
```
...
**Task:** Press `check` to load the data set *city_pm_clean.rds* using `read_rds()` and store it in the variable `city_pm`. Then display the first few rows with `head()` to preview the structure of the data.

```{r "15_7"}
# Load the city-level pollution data set and preview the first rows
```

**Note**: I have cleaned the data set again by removing unnecessary environmental, centroid, and coastal distance variables to ensure a cleaner output and better presentation.
...
**Task:** Calculate city-level de-meaned `pm25` values. Use `group_by(city_id)` to compute the average `pm25` within each city and subtract it from each observation. Store the residuals as a new variable called `pm_res` in the `city_pm` data set. Then ungroup the data to return to the full data set structure.
...
**Task:** Group the data set by `number_iv` and `year`, and then calculate the average of the de-meaned pollution values `(pm_res)` for each group-year combination. Store the result in a new data set called `collapsed_data` and display the first few rows using `head()`.
...
Quiz: What evidence supports the assumption of common trends before 2015?

(1) The AOD trends in all groups diverge before 2015.
(2) All four groups share a common AOD trend before 2015.
(3) Cities with one monitor experience a sharp decline in AOD before 2015.
(4) The AOD levels remain constant in cities with six monitors after 2015.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Assessing the Validity of Common Trends")
```
...
Quiz: What does the pre-2015 trend indicate about the validity of the DiD approach?

(1) The treatment and control groups followed different trends, violating the parallel trends assumption.
(2) The treatment and control groups exhibited relatively stable trends before 2015, supporting the parallel trends assumption.
(3) The treatment group had significantly higher pollution levels before 2015, making the analysis invalid.
(4) The parallel trends assumption is not necessary for a DiD analysis.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("parallel_trends_validation")
```
...
So let us extend our model by incorporating additional control variables to improve the robustness of our parallel trends assumption check. These controls help account for potential confounding factors that may influence air pollution levels:
...
Let us begin the analysis by estimating the difference-in-differences (DiD) regression with both control variables (CV) and fixed effects (FE).
...
**Task:** Fill in the blanks to create a new data set where all control variables are held constant.

```{r "16_8"}

# Create a new data set with constant control variables
dat.0 <- dat.pt %>%
...
```{r "16_10",fig.width=14, fig.height=5}
#  Compute Mean Values for Annotations
...
Quiz: What is the main takeaway from comparing the fixed effects (FE) model to the fixed effects plus control variables (FE + CV) model in the parallel trends plots?

(1) The overall patterns are similar, but the inclusion of control variables slightly shifts the level of pollution in both groups.
(2) The control group shows a completely different trend in the FE + CV model.
(3) The treatment effect disappears when control variables are added.
(4) The FE + CV model shows a stronger violation of the parallel trends assumption.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Comparing FE and FE with Controls")
```
...
Quiz: What is the key difference between the authors' DiD model and ours in terms of fixed effects (FE) and control variables (CV)?

(1) Our model includes more fixed effects than the authors' model.
(2) The authors' model estimates the impact of precipitation, temperature, and incentives separately, while our model absorbs them into fixed effects.
(3) There is no difference in how fixed effects and control variables are handled.
(4) The authors' model absorbs precipitation, temperature, age, and incentives into Fixed Effects, while our model treats them as explicit control variables.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Model Specification Issue")
```
...
Quiz: How does the estimated Monitor Effect change across the three model specifications?

(1) It disappears in the FE + CV model.
(2) It becomes insignificant in the FE-only model.
(3) It becomes stronger in the authors' model compared to the others.
(4) It fluctuates randomly with no pattern.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("comparison results")
```
...
modelsummary(
  list("Authors' Model FE+CV ENF" = did_model_2),   
  coef_map = c("Monitor_Effect" = "Monitor Effect"),
  stars = TRUE, 
  gof_omit = "IC|Log|Adj|F",
  output = "kableExtra"
...
Quiz: Does the positive Monitor Effect coefficient **(0.151)** align with expectations regarding the impact of monitors on enforcement actions?

(1) Yes, because installing monitors is expected to increase regulatory enforcement actions by providing better data on pollution violations.
(2) No, because installing monitors should not influence enforcement actions directly.
(3) No, because we expected a decrease in enforcement actions after installing monitors.
(4) Yes, because monitors are expected to reduce enforcement actions by improving compliance.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Expected_Impact_of_Monitors_on_Enforcement")
```
...
Quiz: Recall *Chapter 4.1*. The central government implemented a systematic method for distributing pollution monitors across cities. What key factors influenced this allocation?

(1) Population size and geographical area.
(2) Historical pollution levels.
(3) The number of previous enforcement actions.
(4) Random assignment across all cities.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Identification Strategy")
```
...
To further clarify the identification strategy proposed by the authors, I constructed a **Directed Acyclic Graph (DAG)**. The graph summarizes the key causal relationships in the model. The number of installed monitors, *X*, may be endogenous due to unobserved city-specific factors, *U*, that also influence enforcement or pollution outcomes, *Y*. To address this issue, the centrally assigned number of monitors, *Z*, is used as an instrument. Since *Z* is based on predetermined allocation rules and is unrelated to local conditions, it provides exogenous variation in *X*. The DAG illustrates that *Z* affects *Y* only through *X*, supporting the validity of the IV approach discussed by the authors.


**Task:** Click `check` to generate the instrumental variable (IV) DAG. 

```{r "17_1",echo=TRUE, fig.width=12, fig.height=7}
# Load package
library(igraph)
...
Quiz: Why is the instrument Assigned Monitors *Z* considered valid in the context of this model?

(1) Because Assigned Monitors Z influence the number of Installed Monitors X, and X affects the Outcome Y, while Z is assumed to be independent of unobserved confounders U.
(2) Because Assigned Monitors Z directly determine the Outcome Y, such as pollution levels or enforcement intensity.
(3) Because the number of Installed Monitors X is randomly assigned across cities.
(4) Because unobserved factors U influence only the Outcome Y, but not the number of Installed Monitors X.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Understanding the IV DAG")
```
...
Quiz: Does the number of government-assigned pollution monitors significantly influence the number of installed monitors?

(1) Yes, assigned monitors strongly predict installed monitors.
(2) No, there is no significant relationship.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("instrument_relevance_1")
```
...
**Task:** Just `check` the chunk to load the data set.
...
Quiz: What is the calculated First-Stage F-Statistic for post1:number_iv?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("first_stage_f_stat_value")
```
...
Quiz: What is the main difference between the IV and the regular DiD estimates of the monitoring effect?

(1) The IV estimates show a weaker effect than the DiD estimates, suggesting that monitoring has less impact than initially thought.
(2) The IV and DiD estimates are identical, meaning endogeneity does not influence the results.
(3) The IV estimates show a stronger effect of monitoring on both pollution reduction and enforcement compared to the DiD estimates.
(4) The IV estimates show no significant relationship between monitoring and either pollution or enforcement.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("IV_vs_DiD_results")
```
...
To better understand the role of age in shaping political incentives, we first examine the age distribution of local officials in our data set. This allows us to assess the variation in career stages and how it aligns with promotion incentives.
...
Quiz: Based on the summary statistics of the `age` variable, what can we conclude about the typical career stage of officials in the data set?

(1) The majority of officials are in their mid-50s.
(2) Most officials are in their early 40s.
(3) Officials are evenly distributed across all ages.
(4) The data set primarily consists of younger officials under 40.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("age_distribution")
```
...
Quiz: Why is age 58 chosen as the reference value in this analysis?

(1) It is an arbitrary midpoint in the analyzed age range.
(2) It is the oldest age in the analyzed range.
(3) It is the youngest age in the analyzed range.
(4) It marks a pivotal point where promotion incentives begin to weaken.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Understanding the Reference Value")
```
...
Quiz: What do the results from the graph suggest about the relationship between monitoring `(post1)` and air pollution `(pm25)` across mayoral ages?

(1) Monitoring has a uniform effect on air pollution across all mayoral ages.
(2) Monitoring is most effective for mayors aged 59 and above.
(3) Monitoring has the strongest effect on reducing air pollution for mayors aged 57 and below.
(4) Monitoring has no significant impact on air pollution regardless of mayoral age.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Interpreting the Results from the Graph")
```
...
Quiz: What does a positive value of $\beta_2$ indicate in the regression model?

(1) The reassignment of monitors has no impact on data accuracy
(2) The alignment between AOD and PM2.5 improves after the reassignment of monitors
(3) The relationship between AOD and PM2.5 weakens after the reassignment
(4) Pollution levels decrease significantly due to reassignment

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("interpretation_of_beta2")
```
...
Let us now take a closer look at the results to see how the reassignment impacted the alignment between AOD and PM2.5.

**Task:** Click on `check` to load the data set *monitor_api.rds* and display its first few rows to get an overview of the data.
...
Quiz: What does the coefficient **0.315** for `AOD` suggest about its relationship with `log_pm25api`?

(1) A one-unit increase in AOD is associated with a 0.315% increase in PM2.5.
(2) AOD has no statistically significant relationship with PM2.5.
(3) A one-unit increase in AOD is associated with a 31.5% increase in PM2.5.
(4) A one-unit increase in AOD decreases PM2.5 by 31.5%.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Interpreting the Coefficient of AOD")
```

After extracting the coefficient and standard error for `AOD`, we can interpret the coefficient value of **0.315** as follows: a one-unit increase in `AOD` is associated with a **31.5%** increase in `log_pm25api`, indicating a positive and statistically significant relationship between satellite-based pollution measurements (`AOD`) and ground-level PM2.5 data (`log_pm25api`).
...
The coefficient for `AOD` in this regression is **0.273**, indicating the elasticity between satellite-based pollution measurements (`AOD`) and ground-level PM2.5 data (`log_pm25api`). This value tells us that a one-unit increase in `AOD` is associated with a **27.3%** increase in ground-level PM2.5 measurements.
...
Quiz: What does the **0.10** increase in elasticity after the reassignment indicate?

(1) The reassignment improved the alignment between AOD and PM2.5 measurements by approximately 37%.
(2) The reassignment led to a 37% decrease in the quality of monitoring information.
(3) The reassignment caused a significant increase in data manipulation by local governments.
(4) The reassignment had no impact on the alignment between satellite and ground-level pollution data.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Elasticity Increase after Reassignment")
```
...
**Task:** Click on `check` to filter the data set for background monitors and run the regression model to estimate the effect of AOD on log-transformed PM2.5 levels. 
...
results <- broom::tidy(model_3) %>%
  ...
  select(term, Coefficient, Std_Error)
...
Quiz: What does the stronger elasticity between satellite-based AOD and PM2.5 readings at background monitors suggest?

(1) Background monitors were more affected by reassignment than the main sample.
(2) The increase in elasticity suggests that satellite data became less reliable.
(3) Background monitors were more manipulated than the main sample before reassignment.
(4) Background monitors were likely less manipulated from the start, leading to a stronger natural correlation with satellite data.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("elasticity_difference")
```
...
Quiz: What does the coefficient for `Monitors × Reassigned` **(−0.0146)** suggest about local governments' efforts after the reassignment of monitoring responsibilities?

(1) Local governments exerted more effort to enforce environmental regulations, leading to reduced pollution.
(2) Local governments decreased their effort to reduce pollution after the reassignment.
(3) The reassignment had no significant impact on pollution reduction efforts.
(4) Pollution levels increased after the reassignment due to reduced enforcement.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Impact of Monitors × Reassigned on Pollution Reduction")
```
...
In the second exercise, we presented and prepared the core data sets used throughout the thesis. This involved loading and cleaning firm-level enforcement data, exploring key variables such as pollution exposure, regulatory actions, and firm characteristics, and preparing the data for regression analysis using tidyverse tools in R.
...
World Health Organization. (2021). WHO global air quality guidelines: Particulate matter (PM₂.₅ and PM₁₀), ozone, nitrogen dioxide, sulfur dioxide and carbon monoxide. Retrieved April 10, 2025, from https://www.who.int/publications/i/item/9789240034228
